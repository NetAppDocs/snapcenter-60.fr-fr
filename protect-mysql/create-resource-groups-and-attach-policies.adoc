...
|===
+
image:../media/application_settings.gif["réglage de l'application"]
...
NOTE: Les paires clé-valeur personnalisées sont prises en charge pour les systèmes plug-in MySQL Linux et ne sont pas prises en charge pour la base de données MySQL enregistrée en tant que plug-in Windows centralisé.
...
.. Dans la colonne configurer les plannings, cliquez sur * *image:../media/add_policy_from_resourcegroup.gif[""]pour la stratégie que vous souhaitez configurer.
