...
Si vous souhaitez ajouter l'hôte de plug-in dans SnapCenter s'exécutant sur un hôte Linux, vous devez obtenir le fichier de checksum à l'emplacement suivant : _/opt/NetApp/snapcenter/SnapManagerWeb/Repository_.
...
.Avant de commencer
Vous devez avoir activé MFA. Pour plus d'informations sur l'activation de MFA, reportez-vous à la section link:../install/enable_multifactor_authentication.html["Activer l'authentification multifacteur"]
