...
+
Vous pouvez créer une stratégie en sélectionnant * * *image:../media/add_policy_from_resourcegroup.gif[""]pour lancer l'assistant de stratégie.
...
.. Dans la boîte de dialogue *Ajouter des plannings pour la stratégie* `policy_name` , configurez le planning, puis sélectionnez *OK*.
+
Voici `policy_name` le nom de la stratégie que vous avez sélectionnée.
